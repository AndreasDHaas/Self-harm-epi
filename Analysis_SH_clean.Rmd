...
pacman::p_load("cowplot", "data.table","dplyr","DiagrammeR", "DiagrammeRsvg", "DT",
               ...
               "REDCapR", "Rmisc","readr","rsvg", "readxl","rstpm2",  #"rollRegres",
...
pasteCI = function (x,y) paste0("(",x, " - ", y, ")")
...
dir.create("Output/IncidenceSH", showWarnings = FALSE, recursive = TRUE)
...
Participants were followed from the entry to the insurance scheme or age 10 year, whichever occurred first, until the exit from the insurance scheme, closure of vital registration database, or death, whichever occurred first. The median follow-up time was `r FU_sum[1] ` years (IQR `r FU_sum[2] ` to `r FU_sum[3] ` years).
...
dd_wide$Self_harm_3 = fct_recode(factor(dd_wide$Self_harm_3), 
                                 "No event"="0", 
                                 "Event of undetermined intent"="1", 
                                 "Self-harm event"="2")
...
dd_tbl = dd_wide %>% mutate(hiv1_n = (1-hiv1_y),
                     ...
                     select(sex, age_start_cat4, age_start,   
                            hiv1_n, hiv1_y,
                            md1_y,
...
N = lu(dd_wide$id)
label.total = paste0("**Total**, N=", round(N/10^6), ",", 
                     floor((N - 10^6*floor(N/10^6))/ 10^3), ",", N-10^3*floor(N/10^3))
...
dd_tbl = dd_wide %>% 
  mutate(hiv1_n = (1-hiv1_y),
         died = 1*(death_y=="Mortality"),
         age_start_cat4 = cut(age_start10, breaks = c(10,15,25,40,100), right=FALSE,
                              labels = c("10-14", "15-24","25-39", "40+")),
         ## Set un-linked deaths to NA
         cause = as.character(cause)) %>%
  mutate(cause = ifelse(linked==0, NA, cause),
         died = ifelse(linked==0, NA, died)) %>% 
  mutate(cause = factor(cause, levels=c("No death","Natural causes","Unnatural causes","Unknown"))) %>%
  select(scheme, 
         sex, age_start_cat4, age_start,   
         hiv1_n, hiv1_y,
         md1_y,
         org1_y, su1_y, psy1_y, bp1_y, dep1_y, anx1_y,
         per1_y, oth1_y,
...
# Convert to flextable 
flextable1 = gtsummary::as_flex_table(table1b)
...
setwd(path_results)
print(doc, target = "Tables/Table1b.docx")
...
#  FIRST SH EVENT
### Incidence of first intentional self-harm events  

- We computed crude incidence rates for the first intentional self-harm events per 100,000 person-years, stratified by sex and HIV status. (The number of individuals with an incident event divided by the total person-years at risk.) 
- We estimated the cumulative incidence of a first intentional self-harm event by sex, HIV status. 
- We used Royston-Parmar flexible parametric survival models to assess the absolute risk of intentional  self-harm. The incidence rates per 100,000 person-years with 95% confidence intervals (CIs) were estimated as a continuous function of age, stratified by sex and HIV status. 
...
##  Person-time at risk for SH events
...
dd_tbl = dd_wide %>% filter(Self_harm_y==1 ) %>%
                     mutate(died = 1*(death_y=="Mortality"),
                            SH_method = case_when(eEvent_sh1 %in% methodx ~ "Highly_lethal", 
                                                                     TRUE ~ "Less_lethal"),
...
if (Export==1){
  setwd(path_figures)
  png("Figure1A.png", width = 1200, height = 700, res = 100); cx = 1.5; L = 3
}
...
```{r, 2 RP survival models by sex - Sensit}
...
if (Export==1){
  setwd(path_figures)
  png("Figure1A_sens.png", width = 1200, height = 700, res = 100)
  L = 3; cx = 1.5
}
...
tableS2 = CI_SH %>% kable(align = c("c"),  table.attr = 'style="width:70%;"',
                         col.names = c("Sex","1","2","3","4","5"),
                         caption = "Cumulative incidence of the first self-harm event at 1-5 years, stratified by sex and age group. Time at risk starts at study entry or entry of the age group.") %>%
...
if(Export==1){
  setwd(path_figures)
  png("Figure1B.png", width = 1200, height = 700, res = 100); cx = 1.5; L = 3
...
incidence_rate_SH = dd_sh %>% 
                    ...
                    dplyr::summarize(Nr_events = sum(SH),
                                     ...
                                     IR_SH = round(10^5 * sum(SH) / sum(FU_time),1),
                                     IR_SH_se = sqrt(10^5 * sum(SH) / sum(FU_time)),
...
# PROGNOSTIC FACTORS of first ISH event 

```{r SH1: Data setup for Cox with TVC}

##  Define tmerge data format
...
##  DEP diagnosis re-set to 0 at diagnosis of BP  
...
- We estimate the adjusted hazard ratios (aHR) with 95% CIs for prognostic factors associated with a first intentional self-harm event using Cox proportional hazards models. Prognostic factors in this model include sociodemographic characteristics, mental disorders, and HIV status. 


```{r ISH prognostic factors: Fully adjusted Cox models}
...
```{r ISH prognostic factors: Partly adjusted Cox models}
...
```{r ISH prognostic factors: Cox results Table - all models}
...
```{r ISH prognostic factors: Export Table of Cox models}

setwd(paste0(path_results,"/Tables"))

# Save the table to an HTML file
save_kable(cox_sh_tbl, file = "Table_Cox_models.html")

# Render the HTML file to Word using rmarkdown
rmarkdown::pandoc_convert("Table_Cox_models.html", to = "docx", output = "Table_Cox_models.docx")
...
```{r ISH prognostic factors: forest plot}
...
```{r Export forest plot}

setwd(path_figures)
png("Figure2_Forest_SH.png", width = 900, height = 1100, res = 100)
...
##  Sensitivity analysis
### Test of interactions age * MHD


```{r ISH Cox models: test interactions age * MHD}

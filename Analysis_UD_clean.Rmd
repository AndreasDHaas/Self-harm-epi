...
4.	Examine the incidence of mortality from unnatural causes among individuals with and without history of an intentional self-harm event.
5.	Evaluate intentional self-harm as predictors of mortality from unnatural causes.
...
- episodesLong7: long table with self-harm events and events of undetermined intent. 
...
dir.create("Output/IncidenceUD", showWarnings = FALSE, recursive = TRUE) 


```

```{r Local Working directories generic}
...
setwd(path_data)
dd_wide = read.dta("analyseWide.dta")  
...
##  AfA vs AfAc scheme
dd_wide = dd_wide %>% mutate(scheme = ifelse(grepl("^[AFA]", patient), "AfA", "AfAc")) 
...
dd_wide = dd_wide %>% filter(linked==1)


##  Censor FU time at "2021-01-26" 
##  closure of vital registration database
dd_wide = dd_wide %>% 
  mutate( end = as.Date(ifelse(end > as.Date("2021-01-26"), as.Date("2021-01-26"), end)) ) 


##  Exclude those enrolled after/at vital registration closure date
dd_wide = dd_wide %>% filter(start10 < end) 
nrow(dd_wide)
...
dd_sui0 = dd_wide %>% filter(!is.na(eSdate_sh1) & 
                                   (death_y == "Mortality") &    # Any death on the day of SH event
                                   (end == eSdate_sh1)) 
id_sui0 = dd_sui0$id 
#id_sui0
...
##  Reviewed claims data for dd_sui1: deaths during hospitalization for the index SH
#   Patient IDs saved in the file "id_sui1.csv"

# setwd(path_data)
# write.csv(id_sui1, "id_sui1.csv")


##  Load IDs of patients with assumed suicides based on review of claims data
##  This dataset needs to be obtained - with recoded IDs 
setwd(path_data)
id_sui1 = read.csv("id_sui1.csv")[,2]


##  Merge these two suicide ID sets
...
After excluding people with unknown sex, unlinked people, and those with start date after end date, there were `rnSH ` intentional self-harm events, followed by `r nSH_UD` unnatural deaths. Of these, `r nSH_UDx ` occurred during the hospital stay  for the index ISH event and were hence considered completed suicides (in the non-ISH group).





#  INCIDENCE OF UNNATURAL DEATH

##  Cause-specific CIF ~ sex, SH 
...
```{r CIF for UD ~ sex + SH: Figure, fig.width = 12, fig.height = 7}
...
if (panelA==1){
  ...
  png("Figure3_CIF_SH.png", width = 1200, height = 700, res = 100)
}
  L = 2; cx = .95
}
...
####  Cumulative incidence at 1-5 yrs


```{r Cumulative incidence by SH at 1-5 yrs}
        
##   TABLE : CIF at 1-5 yrs
    
CI_UD = data.frame(matrix(NA,6,5))

fitF = list(fit_f0, fit_f1)
fitM = list(fit_m0, fit_m1)
fitA = list(fit_a0, fit_a1)
...
for (i in 1:2){
 ...
 for (j in 1:5){
        t = max(time[time<=j])
        CI_UD[4+i,j] = paste0(CIF[time==t], " (",
                            CIlow[time==t], " - ",
                            CIhi[time==t], ")")   
 }}


CI_UD = cbind(rep(c("No SH","SH"),3), CI_UD)

```

```{r Table: CIF for SH at 1-5 yrs}

tableS3_SH = CI_UD %>% kable(align = c("c"),  table.attr = 'style="width:70%;"',
                         col.names = c("Intentional self-harm","1","2","3","4","5"),
                         caption = "Cumulative incidence of unnatural deaths, stratified by sex and self-harm encounter. Time at risk starts at study entry or self-harm encounter.") %>%
                   kable_styling(position = "center") %>%    # full_width = TRUE
                   kableExtra::group_rows(index = c("Female" = 2, "Male" = 2,"All" = 2)) 
...
save_kable(tableS3_SH, file = "TableS3_SH.html")

# Render the HTML file to Word using rmarkdown
rmarkdown::pandoc_convert("TableS3_SH.html", to = "docx", output = "TableS3_SH.docx")
...
### Sensitivity analysis: Time-varying HR for SH (RP flex par model) 

```{r UD ~ sex + SH : 2 par flex models}
...
```{r Plot HR ~ time 2 models, fig.width = 8, fig.height = 6 } 
...
if (Export==1){
  setwd(path_figures)
  png("HR_SH_flex.png", width = 1000, height = 800, res = 100)
  L=3; par(cex=1.6)
}
...
lines(mod_sreg_m_ud, newdata=newdata, type="hr", var="SH", lwd=L, col=colx[2])
lines(mod_sreg_f_ud, newdata=newdata, type="hr", var="SH", lwd=L, col=colx[1])
...
##  Cause-specific CIF of UD ~ sex, SH method 
...
tableS3_SHm = CI_UD %>% kable(align = c("c"),  table.attr = 'style="width:70%;"',
...
```{r, export TableS3_SHm}
#saveTables
setwd(paste0(path_results,"/Tables"))

# Save the table to an HTML file
save_kable(tableS3_SHm, file = "TableS3_SHm.html")

# Render the HTML file to Word using rmarkdown
rmarkdown::pandoc_convert("TableS3_SHm.html", to = "docx", output = "TableS3_SHm.docx")
...
if (Export==1){
  ...
  if (panelB==1){ 
    ...
    png("Figure3_CIF_SHmethod.png", width = 1200, height = 700, res = 100)
  }
  L = 2; cx = 0.95
}
...
```{r UD prognostic factors: Split time @ SH}
...
```{r UD prognostic factors: Split time @ HIV & MHD}
...
####  SH method, HIV, MHD | stratified by sex
...
```{r UD prognostic factors: Univariable Cox models - SHm}
...
```{r UD prognostic factors: Export Cox results Table - SHm}

setwd(paste0(path_results,"/Tables"))

# Save the table to an HTML file
save_kable(cox_ud_tbl, file = "Cox_model_UD_SHm.html")

# Render the HTML file to Word using rmarkdown
rmarkdown::pandoc_convert("Cox_model_UD_SHm.html", to = "docx", output = "Cox_model_UD_SHm.docx")
...
```{r Export forest plot - SHm}

##  Figure Export

setwd(path_figures)
png("Fig4_Forest_UD_SHm.png", width = 1050, height = 500, res = 100)
...
```{r UD prognostic factors: Export Cox results Table - SH }

setwd(paste0(path_results,"/Tables"))

# Save the table to an HTML file
save_kable(cox_ud_tbl, file = "Cox_model_UD_SH.html")

# Render the HTML file to Word using rmarkdown
rmarkdown::pandoc_convert("Cox_model_UD_SH.html", to = "docx", output = "Cox_model_UD_SH.docx")
...
```{r UD prognostic factors: Univariable Cox models - SH}
...
##  Load model coefs
# setwd(path_results)
# cox_ud_coefs_f = as.numeric(read.csv("UD_Cox_coefs_SH_f.csv")[,-1])
# cox_ud_coefs_m = as.numeric(read.csv("UD_Cox_coefs_SH_m.csv")[,-1])
...
```{r Export forest plot - SH}

##  Figure Export

setwd(path_figures)
png("paper1/Fig4_Forest_UD_SHx.png", width = 1050, height = 400, res = 100)
...
##  Sensitivity analysis: Interaction sex * SH(method)

```{r UD: Common model adj with interaction - age as timeline, eval=FALSE}

## Common model with interaction: sex*SH

cox_ud_x1 = coxph(Surv(age_tstart, age_tstop, y) ~ 
                    SH*sex + 
                    ORG + SU + PSY + BP + 
                    DEP_x + ANX + PER + OTH, 
                  data = dd_ud_new)   
...
cox_ud_x2 = coxph(Surv(age_tstart, age_tstop, y) ~ 
                    SH_method3*sex + 
                    ORG + SU + PSY + BP + 
                    DEP_x + ANX + PER + OTH, 
                  data = dd_ud_new)   
...
cox_ud_x3 = coxph(Surv(age_tstart, age_tstop, y) ~ 
                    SH_method3 + sex + 
                    ORG + SU + PSY + BP + 
                    DEP_x + ANX + PER + OTH, 
                  data = dd_ud_new)   
lrtest(cox_ud_x2, cox_ud_x3)

```


```{r UD: Common unadj model with interaction - age as timeline, eval=FALSE}

## Common model with interaction: sex*SH

cox_ud_x1i = coxph(Surv(age_tstart, age_tstop, y) ~ SH*sex, data = dd_ud_new)   
cox_ud_x1i

cox_ud_x1 = coxph(Surv(age_tstart, age_tstop, y) ~ SH + sex, data = dd_ud_new)   
cox_ud_x1
summary(cox_ud_x1)


cox_ud_x2i = coxph(Surv(age_tstart, age_tstop, y) ~ SH_method3*sex, data = dd_ud_new)   
cox_ud_x2i

cox_ud_x2 = coxph(Surv(age_tstart, age_tstop, y) ~ SH_method3 + sex, data = dd_ud_new)  
cox_ud_x2
summary(cox_ud_x2)

lrtest(cox_ud_x2, cox_ud_x2i)
